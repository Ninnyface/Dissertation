...
void Camera::UpdateBallCamera(float dt, Vector3 ballPos) {
	//Update the mouse by how much
	pitch -= (Window::GetMouse()->GetRelativePosition().y);
	yaw -= (Window::GetMouse()->GetRelativePosition().x);
	...
	position = Vector3(ballPos.x - 100*cos((yaw+90)/180*3.14f), ballPos.y + 50, ballPos.z + 100*sin((yaw+90)/180*3.14f));
